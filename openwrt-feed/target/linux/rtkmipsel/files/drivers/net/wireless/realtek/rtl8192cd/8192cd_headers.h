...
extern char rtl8192cd_dlink_log_debug;
extern char rtl8192cd_dlink_acs_debug;
extern char rtl8192cd_dlink_psk_debug;
extern char rtl8192cd_dlink_pmf_debug;
extern char rtl8192cd_dlink_sta_debug;
#endif
...
#ifdef PMF_DEBUGMSG
#ifdef CONFIG_DLINK_WIFI_DEBUG
#define PMFDEBUG(fmt, args...) do {if (rtl8192cd_dlink_pmf_debug) printk("[PMF](%s:%d):"fmt,__func__,__LINE__,## args);} while (0)
#else
...
#if defined(CONFIG_DLINK_WIFI_DEBUG)
#define STADEBUG(fmt, args...) do {if (rtl8192cd_dlink_sta_debug) printk("[STA][%s] (%s:%d):"fmt,priv->dev->name,__func__,__LINE__,## args);} while (0)
#else
#define STADEBUG(fmt, args...) { }
#endif
#ifdef _DEBUG_RTL8192CD_
#ifdef CONFIG_DLINK_WIFI_DEBUG
#define PSKDEBUG(fmt, args...) do {if (rtl8192cd_dlink_psk_debug) printk("[PSK][%s] (%s:%d):"fmt,priv->dev->name,__func__,__LINE__,## args);} while (0)
#else
...
#ifdef CONFIG_DLINK_WIFI_DEBUG
#define ACSDEBUG(fmt, args...) do {if (rtl8192cd_dlink_acs_debug) printk("[ACS][%s]:"fmt,priv->dev->name,## args);} while (0)
#else
...
EXTERN u32 find_rate_dlink(struct rtl8192cd_priv *priv);
#endif
...
#endif
EXTERN void issue_probereq_map(struct rtl8192cd_priv *priv, unsigned char *ssid, int ssid_len, unsigned char *da);
...
EXTERN u32 PHY_GetTxPowerFinalAbsoluteValuemdBm(struct dm_struct *dm, u8 rf_path,	u8 rate, u8 bandwidth, u8 channel);
...
EXTERN unsigned char *construct_multiap_ie(unsigned char *pbuf, unsigned int *frlen, unsigned char profile, unsigned char bss_type);
