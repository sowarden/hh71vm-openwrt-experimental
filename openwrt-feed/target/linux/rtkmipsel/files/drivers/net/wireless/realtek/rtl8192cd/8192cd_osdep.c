...
// TODO: we should modify code below
	...
	if (bcnInt) {
		...
		if(priv->pshare->rf_ft_var.beacon_igi){
			...
			struct dm_struct	*pDM_Odm __maybe_unused = ODMPTR;
#if defined(CONFIG_WLAN_HAL_8822BE) 
...
#ifndef CONFIG_RTL_TDMA_DIG_FIX
#ifdef CONFIG_WLAN_HAL_8197G
	if (GET_CHIP_VER(priv) == VERSION_8197G){
		ODMPTR->tdma_dig_state_number = 10 ;	
	}
#endif 

#ifdef CONFIG_WLAN_HAL_8814BE
	if (GET_CHIP_VER(priv) == VERSION_8814B){
		ODMPTR->tdma_dig_state_number = 20 ;	
	}
#endif
#endif /* CONFIG_RTL_TDMA_DIG_FIX */
...
/* No idea how to process AMSDU shortcut with MESH or WDS, so disable AMSDU shortcut when MESH or WDS enabled */
	...
	// dlink fix: don't overwrite userspace value
...
u32 PHY_GetTxPowerFinalAbsoluteValuemdBm(struct dm_struct *dm, u8 rf_path,
					u8 rate, u8 bandwidth, u8 channel)
{
	u8 tx_power_dbm = 0;
	struct rtl8192cd_priv *priv = dm->priv;
...
int rtl8192cd_open(struct net_device *dev)
{
	...
	struct hash_table		*sae_peer_hex64_fail_table = NULL;
	#endif
...
// init mib from cfg file, we only need to load cfg file once - chris 2010/02
...
#endif
	{
		...
		// #36562 dlink, don't needed if using iwpriv to set mibs
		memcpy((void *)dev->dev_addr, priv->pmib->dot11OperationEntry.hwaddr, 6);
#endif
	}
#endif
...
// check phyband and channel match or not
    ...
    if (rc) {
		...
		#if 1  // Mark.CS_debug.1498
		log("[%s] init sae_peer_hex64_fail_table \n ",priv->dev->name);
		sae_peer_hex64_fail_table = (struct hash_table*)kmalloc(sizeof(struct hash_table), GFP_ATOMIC);
		if(!sae_peer_hex64_fail_table)
		{
			rc = -ENOMEM;
			printk("allocate sae_peer_hex64_fail_table error!!\n");
			goto free_res;
		}
		memset((void*)sae_peer_hex64_fail_table, 0, sizeof(struct hash_table));
		#endif

		log("[%s] init sae_blacklist_table \n ",priv->dev->name);
		...
		#if 1  // Mark.CS_debug.1498
		rc = init_hash_table(sae_peer_hex64_fail_table,
				SAE_TABLE_SIZE, MACADDRLEN,
				sizeof(struct candidate),
				crc_hashing,
				search_default, insert_default,
				delete_default,traverse_default);
		if(rc == HASH_TABLE_FAILED){
				log("init_hash_table sae_peer_hex64_fail_table error \n");   // Mark.CS_debug.1498
		}
		priv->sae_peer_hex64_fail_table = sae_peer_hex64_fail_table;
		log("init sae_peer_hex64_fail_table done");
		#endif
...
//			priv->ps_ctrl = 0x11;
		...
		if (priv->pmib->miscEntry.tcpack_acc)
		{
			...
			priv->tcpack_timer.function = (void (*)(unsigned long))check_tcp_ack_timeout;
...
#ifdef CONFIG_DLINK_WIFI_TENDA_8812F_INTERFERENCE_PATCH
	/* Interferrence issues 05152020 */
	if (GET_CHIP_VER(priv) == VERSION_8812F)
	{
		REG32(0xb8b01000)=0x48440801;
		delay_ms(20);
		RTL_W32(0x3f4,0x4844);
		delay_ms(10);
		RTL_W32(0x3f8,0x28);
		delay_ms(10);
	}
#endif /* CONFIG_DLINK_WIFI_TENDA_8812F_INTERFERENCE_PATCH */
...
free_res:
	...
	#if 1  // Mark.CS_debug.1498
	if (sae_peer_hex64_fail_table){
		log("[%s]rm sae_peer_hex64_fail_table",priv->dev->name);
		remove_hash_table(sae_peer_hex64_fail_table);
		kfree(sae_peer_hex64_fail_table);
	}
	#endif
...
*/
	...
	#if 1 // Mark.CS_debug.1498
	if(priv->sae_peer_hex64_fail_table){
		log("[%s]rm sae_peer_hex64_fail_table",priv->dev->name);
		remove_hash_table(priv->sae_peer_hex64_fail_table);
		kfree(priv->sae_peer_hex64_fail_table);
	}
	#endif
...
#ifdef SUPPORT_ROAMING
	roam_init_adapter(priv);
#endif
...
#if defined(CONFIG_RTL8197_MACH_HH71VM)
	for (wlan_index = 0; wlan_index < ARRAY_SIZE(wlan_device); wlan_index++)
#elif (defined(__KERNEL__) && !defined(CONFIG_WIRELESS_LAN_MODULE)) || defined(__OSK__)
	for (wlan_index = 0; wlan_index < ARRAY_SIZE(wlan_device); wlan_index++)
#else
