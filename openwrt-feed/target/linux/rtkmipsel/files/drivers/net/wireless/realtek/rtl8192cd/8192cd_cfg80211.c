...
#if /*defined(OPENWRT_CC) || */(LINUX_VERSION_CODE >= KERNEL_VERSION(3, 18, 0))
...
#else
					cfg80211_cac_event(priv->dev, NL80211_RADAR_CAC_ABORTED, GFP_KERNEL);
#endif
				}
			}
				   ...
				   struct key_params *params)			   
{
	...
	if (FT_ENABLE && mac_addr) {
		struct stat_info *pstat = get_stainfo(priv, mac_addr);
		if (pstat == NULL) {
			pstat = alloc_stainfo(priv, mac_addr, -1);
			if (pstat == NULL) {
				FTDEBUG("Exceed the upper limit of supported clients...\n");
				return -1;
			}

			FTDEBUG("alloc ft-sta\n");
...
void realtek_del_station(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
    ...
    if(pstat->state & WIFI_ASOC_STATE){
        ...
        NDEBUG3("\n");        
    	issue_disassoc(priv, pstat->cmn_info.mac_addr, _RSON_AUTH_NO_LONGER_VALID_);
    }
...
static void realtek_set_band(struct rtl8192cd_priv *priv, struct cfg80211_beacon_data *beacon, struct cfg80211_chan_def *chandef)
{
	...
	u8 dot11FTMobilityDomainID[2] = {0}, dot11FTR0KeyHolderID[49] = {0};
#endif
	...
	if (beacon->tail_len)
	{
		...
		if (is_ht_enable)
		{
			p = get_ie(beacon->tail, _EXTENDED_CAP_IE_, &len, beacon->tail_len);
...
check_end:
...
#endif
#ifdef CONFIG_IEEE80211R
		p = get_ie(beacon->tail, _MOBILITY_DOMAIN_IE_, &len, beacon->tail_len);
		if (p) {
		...
		/* sync countrystr */
		p = get_ie(beacon->tail, _COUNTRY_IE_, &len, beacon->tail_len);
		if (p) 
		{
			u8 *str = (p + 2);
	...
	if((OPMODE&WIFI_AP_STATE) && info->chandef.chan->dfs_state == NL80211_DFS_AVAILABLE) {
		...
		if (timer_pending(&priv->DFS_timer))
			...
			if (GET_HCI_TYPE(priv) == RTL_HCI_USB || GET_HCI_TYPE(priv) == RTL_HCI_SDIO) {
            	priv->DFS_timer.function = pre_rtl8192cd_DFS_timer;
...
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
			if (GET_HCI_TYPE(priv) == RTL_HCI_USB || GET_HCI_TYPE(priv) == RTL_HCI_SDIO) {
            	priv->dfs_det_chk_timer.function = pre_rtl8192cd_dfs_det_chk_timer;
...
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
		if (GET_HCI_TYPE(priv) == RTL_HCI_USB || GET_HCI_TYPE(priv) == RTL_HCI_SDIO) {
        	priv->ch_avail_chk_timer.function = pre_rtl8192cd_ch_avail_chk_timer;
...
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
		if (GET_HCI_TYPE(priv) == RTL_HCI_USB || GET_HCI_TYPE(priv) == RTL_HCI_SDIO) {
        	priv->DFS_TXPAUSE_timer.function = pre_rtl8192cd_DFS_TXPAUSE_timer;
					 ...
					 struct cfg80211_update_ft_ies_params *ftie)
{
	...
	mem_dump(__func__, ftie->ie, ftie->ie_len);
	...
	*cookie = priv->mgmt_tx_cookie++;
	...
	if (ieee80211_is_auth(mgmt->frame_control)) {
		...
		if (pstat->ft_state && le16_to_cpu(mgmt->u.auth.auth_alg) == _AUTH_ALGM_FT_) {
			FTDEBUG("issue ft-auth packet\n");
		
			sta_info = pstat->wpa_sta_info;

			limit = params->len - (ieee80211_hdrlen(mgmt->frame_control) + sizeof(mgmt->u.auth));
			pbuf = params->buf + (ieee80211_hdrlen(mgmt->frame_control) + sizeof(mgmt->u.auth));			
		...
		if (pstat->ft_state == state_ft_assoc) {
			FTDEBUG("issue ft-reassoc-rsp packet\n");

			sta_info = pstat->wpa_sta_info;
			
			pbuf = params->buf + (ieee80211_hdrlen(mgmt->frame_control) + sizeof(mgmt->u.reassoc_resp));
...
#ifdef CONFIG_IEEE80211R	
		if (mgmt->u.action.category == _FAST_BSS_TRANSITION_CATEGORY_ID_) {
			u8 *action_ie = params->buf + ieee80211_hdrlen(mgmt->frame_control);
			u32 len = params->len - ieee80211_hdrlen(mgmt->frame_control);

			FTDEBUG("issue ft-action packet\n");
			issue_ft_action(priv, pstat, action_ie, len);
		}
#endif /* CONFIG_IEEE80211R */
	} else if (ieee80211_is_assoc_resp(mgmt->frame_control)) {
#ifdef CONFIG_IEEE80211R
		if (pstat->ft_state == state_imd_assoc) {
			FTDEBUG("issue rf-assoc-rsp packet\n");

			sta_info = pstat->wpa_sta_info;
			
			pbuf = params->buf + (ieee80211_hdrlen(mgmt->frame_control) + sizeof(mgmt->u.assoc_resp));
		...
		NDEBUG("not support frame control=0x%x\n", le16_to_cpu(mgmt->frame_control));
		return -EPERM;
	}
...
{	
	...
	unsigned int dot11nUse40M = priv->pmib->dot11nConfigEntry.dot11nUse40M;
	unsigned int dot11channel = priv->pmib->dot11RFEntry.dot11channel;
	unsigned int dot11n2ndChOffset = priv->pmib->dot11nConfigEntry.dot11n2ndChOffset;
	unsigned int centerchannel = get_center_channel(priv, dot11channel, dot11n2ndChOffset, 0);
	int freq;
