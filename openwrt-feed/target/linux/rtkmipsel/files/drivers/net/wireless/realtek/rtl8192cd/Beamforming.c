	...
	)
{
	...
	if((GET_CHIP_VER(priv)== VERSION_8822B) || (GET_CHIP_VER(priv)== VERSION_8812F) || (GET_CHIP_VER(priv) == VERSION_8814B)) {
		...
		pBeamInfo->BeamformingTimer.function = (void (*)(unsigned long))beamform_SoundingTimerCallback;
		pBeamInfo->BeamformingTimer.data = (unsigned long)priv;

		init_timer(&pBeamInfo->BFSoundingTimeoutTimer);
		pBeamInfo->BFSoundingTimeoutTimer.function = (void (*)(unsigned long))beamform_SoundingTimeout;
