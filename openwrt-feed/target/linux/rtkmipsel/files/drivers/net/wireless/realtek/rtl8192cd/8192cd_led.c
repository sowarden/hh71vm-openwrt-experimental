...
static void set_sw_LED0(struct rtl8192cd_priv *priv, int flag)
{
    ...
    else if(GET_CHIP_VER(priv) == VERSION_8197F || GET_CHIP_VER(priv) == VERSION_8197G) {
#ifdef CONFIG_DLINK_WIFI_2G_LED_GPIO_CONNECTED
        if(flag) /* led off*/
            gpio_set_value(CONFIG_DLINK_WIFI_2G_LED_GPIO_PIN, 1);
        else  /*led on*/
            gpio_set_value(CONFIG_DLINK_WIFI_2G_LED_GPIO_PIN, 0);
#endif
    }
    #endif
    #if defined(CONFIG_WLAN_HAL_8822BE) 
    else if(GET_CHIP_VER(priv) == VERSION_8822B) {
#ifdef CONFIG_DLINK_WIFI_5G_LED_GPIO_CONNECTED
#ifdef CONFIG_DLINK_WIFI_5G_LED_GPIO_HIGH
	if (!flag) /* led off */
#else
	if (flag) /* led off */
#endif
		gpio_set_value(CONFIG_DLINK_WIFI_5G_LED_GPIO_PIN, 1);
	else /* led on */
		gpio_set_value(CONFIG_DLINK_WIFI_5G_LED_GPIO_PIN, 0);
#else
        if(flag) /* led off*/
            RTL_W32(0x60, (RTL_R32(0x60) & 0xFFFFFEFF) | BIT8);
        else  /*led on*/
...
void enable_sw_LED(struct rtl8192cd_priv *priv, int init)
{
	...
	int err = 0;
	#ifdef CONFIG_DLINK_WIFI_2G_LED_GPIO_CONNECTED
        err = gpio_request_one(CONFIG_DLINK_WIFI_2G_LED_GPIO_PIN, GPIOF_OUT_INIT_HIGH | GPIOF_EXPORT_DIR_FIXED, "2G Wifi");
	if(err)
		printk("WiFi 2G LED gpio request error(%d)\n", err);
	#endif
	#endif
#endif
    } else
...
#if defined(CONFIG_WLAN_HAL_8822BE)
    if (GET_CHIP_VER(priv) == VERSION_8822B) {
	#if !defined(CONFIG_DLINK_WIFI_5G_LED_GPIO_CONNECTED)
        RTL_W32(LEDCFG, (RTL_R32(LEDCFG) & (~LED2EN)));
        RTL_W32(0x60, (RTL_R32(0x60) | BIT16 | BIT24));
	#else
#ifdef CONFIG_DLINK_WIFI_5G_LED_GPIO_HIGH
	gpio_request_one(CONFIG_DLINK_WIFI_5G_LED_GPIO_PIN, GPIOF_OUT_INIT_LOW | GPIOF_EXPORT_DIR_FIXED, "5G Wifi");
#else
	gpio_request_one(CONFIG_DLINK_WIFI_5G_LED_GPIO_PIN, GPIOF_OUT_INIT_HIGH | GPIOF_EXPORT_DIR_FIXED, "5G Wifi");
#endif /* CONFIG_DLINK_WIFI_5G_LED_GPIO_HIGH */
    	#endif    
    } else
#endif
#if defined(CONFIG_WLAN_HAL_8812FE)
...
#ifdef CONFIG_DLINK_WIFI_LED_STATE_FIX
	priv->pshare->LED_ToggleStart = LED_OFF;
#endif
...
#ifdef CONFIG_DLINK_ALLOW_LED_DISABLE_WIFI_FIX
void control_wireless_led_by_num(int wifi_num, int enable)
{
	char buf[IFNAMSIZ];
	snprintf(buf, IFNAMSIZ, "wlan%d", wifi_num);
	struct net_device *dev = dev_get_by_name(&init_net, buf);
	if (dev) {
		struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
		control_wireless_led(priv, enable);
		priv->pshare->LED_en = enable; //dkonev
	}
}
#else
void control_wireless_led_by_num(int wifi_num, int enable)
...
#endif
EXPORT_SYMBOL(control_wireless_led_by_num);

/* for WPS management */
void set_5G_chip_led(int enable)
{
