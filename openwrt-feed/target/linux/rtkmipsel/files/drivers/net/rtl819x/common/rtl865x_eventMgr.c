		...
		CTAILQ_FOREACH(event, &eventLayerList->eventHead, next)
		{
			...
			{	
				if(event->event_action_fn!=NULL)
				{
					...
					{
					#if defined(CONFIG_SMP)
						if ((event->eventId == EVENT_UPDATE_MCAST) ||(event->eventId == EVENT_UPDATE_MCAST6)) 
						{
					#if defined(CONFIG_HW_MCAST_DEBUG)
						    printk("HW_MCAST:%s:%d eventId:%x event->eventId:%x START\n",	__func__, __LINE__, eventId, event->eventId);
					#endif
					...
					#if defined(CONFIG_HW_MCAST_DEBUG)
						printk("HW_MCAST:%s:%d eventId:%x event->eventId:%x END\n",__func__, __LINE__, eventId, event->eventId);
					#endif
					}
					#endif
