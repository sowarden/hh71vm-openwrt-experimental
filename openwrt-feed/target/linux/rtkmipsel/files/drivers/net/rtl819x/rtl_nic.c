...
#define HH71VM_HWSETTING_OFFSET		0x20000
#define HH71VM_H601_MAC_OFFSET		7

static int hh71vm_read_flash_mac(int idx, unsigned char *mac)
{
	const volatile unsigned char *p =
		(const volatile unsigned char *)(KSEG1ADDR(BSP_SPIC_AUTO_BASE) +
						 HH71VM_HWSETTING_OFFSET);
	int i;

	if (p[0] != 'H' || p[1] != '6' || p[2] != '0' || p[3] != '1')
		return -1;

	p += HH71VM_H601_MAC_OFFSET + idx * ETHER_ADDR_LEN;
...
static void hh71vm_apply_flash_macs(void)
{
	unsigned char mac[ETHER_ADDR_LEN];
	int i;

	for (i = 0; vlanconfig[i].vid != 0; i++) {
		if (hh71vm_read_flash_mac(i, mac) != 0) {
			rtlglue_printf("hwsetting: MAC %d not found, keeping default\n", i);
			continue;
		}
		memcpy(vlanconfig[i].mac.octet, mac, ETHER_ADDR_LEN);
		rtlglue_printf("hwsetting: eth%d MAC %02x:%02x:%02x:%02x:%02x:%02x\n",
...
#endif
#if defined(CONFIG_NET_SCHED) && !defined(CONFIG_OPENWRT_SDK)
