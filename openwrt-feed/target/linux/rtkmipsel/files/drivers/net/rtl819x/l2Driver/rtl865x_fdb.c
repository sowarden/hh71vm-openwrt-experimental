...
int32 rtl_add_ext_fdb_entry(const unsigned char *addr)
{
	return rtl865x_addExtFDBEntry(addr, PKTHDR_EXTPORT_P3);
}

	
int32 rtl_del_ext_fdb_entry(const unsigned char *addr)
{
	ether_addr_t *macAddr;
	int32 column, ret = FAILED;
	rtl865x_tblAsicDrv_l2Param_t	fdbEntry;
	...
	macAddr = (ether_addr_t *)(addr);
	if (rtl865x_Lookup_fdb_entry(RTL_LAN_FID, macAddr, FDB_STATIC, &column, &fdbEntry) == SUCCESS)
	{
		if ((fdbEntry.memberPortMask & (1<<PKTHDR_EXTPORT_P3)) != 0)
		{
			ret = _rtl865x_delFilterDatabaseEntry(RTL865x_L2_TYPEIII, RTL_LAN_FID, macAddr);
		}
	}
